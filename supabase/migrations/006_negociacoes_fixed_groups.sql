-- Garante o grupo fixo "Perdidos" em boards de negociações existentes
INSERT INTO board_groups (board_id, name, color, position)
SELECT b.id, 'Perdidos', '#F44545', COALESCE((
  SELECT MAX(g.position) + 1 FROM board_groups g WHERE g.board_id = b.id
), 2)
FROM boards b
WHERE b.slug = 'negociacoes'
AND NOT EXISTS (
  SELECT 1 FROM board_groups g
  WHERE g.board_id = b.id
  AND lower(trim(g.name)) = 'perdidos'
);

-- Atualiza seed para novas organizações
